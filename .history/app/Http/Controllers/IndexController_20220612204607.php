...
class IndexController extends Controller
{
    ...
    public function home()
    {
        ...
        // Get all stroy
        $story = Story::orderBy('id', 'DESC')->where('status', 0)->paginate(24);
        // Get story hot
        $hotStory = Story::where('label_hot', 0)->take(10)->get();
        return view('pages.home')->with(compact('category', 'story', 'hotStory'));
    }
    ...
    public function watchcategory($slug)
    {
        ...
        // Get story hot
        $hotStory = Story::where('label_hot', 0)->take(10)->get();
        return view('pages.category')->with(compact('category', 'category_id', 'category_name', 'story', 'hotStory'));
    }
    ...
    public function watchstory($slug)
    {
        ...
        $storyByCategory = Story::with('category')->where('category_id', $story->category->id)->whereNotIn('id', [$story->id])->take(8)->get();
    ...
    public function watchchapter($slug)
    {
        ...
        $all_chapter = Chapter::with('story')->where('story_id', $story->story_id)->orderBy('id', 'ASC')->get(); // Get all chapter
    ...
    public function search()
    {
        ...
        $keys = $_GET['search'];
        $story = Story::with('category')->where('title', 'LIKE', '%' . $keys . '%')->orWhere('description', 'LIKE', '%' . $keys . '%')->orWhere('author', 'LIKE', '%' . $keys . '%')->get();
        // Get story hot
        $hotStory = Story::where('label_hot', 0)->take(10)->get();
        return view('pages.search')->with(compact('category', 'keys', 'story', 'hotStory'));
    }
}
