        ...
        <div class="col-sm-12">
            <div class="content-single">
                ...
                <div class="chapter-btn">
                    <a href="{{url('xem-chapter/'.$prevChapter)}}" class="btn btn-success {{ $chapter->id == $min_id->id ? 'isDisable' : '' }}">Prev</a>
                    <select name="" id="select_chapter">
                        @foreach($all_chapter as $key => $value)
                        <option value="{{url('xem-chapter/'.$value->slug)}}">{{$value->title}}</option>
                        @endforeach
                    </select>
                    <a href="{{url('xem-chapter/'.$nextChapter)}}" class="btn btn-success {{$chapter->id == $max_id->id ? 'isDisable' : ''}}">Next</a>
                </div>
                <article>
                    {!! $chapter->content !!}
                </article>
